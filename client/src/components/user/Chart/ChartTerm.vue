    ...
    <main>
        <div class="container">
            <form novalidate @submit.stop.prevent>
                <div v-if="normalshow" class="form-inline">
                    <div class="form-group col-md-6 normalmod">
                        ...
                        <div class="col-md-4">
                        <input type="date" v-model="indate1" id="indate1" class="form-control indate1normalmod">
                        </div>
                    </div>
                ...
                <div v-if="pbshow" class="form-inline">
                    <div class="form-group col-md-4">
                        <label for="gubun2" class="col-md-4 col-form-label">대분류 </label>
                        <div class="col-md-8">
                            <select v-model.number="pbgubun2" id="gubun2" v-on:change="onLargeChange" class="form-control largeselmod"> 
                                <option value=3>PB</option>
                            </select>
                        </div>
                    </div>
            ...
            <ul class="list-group">
                <li v-for="clist in chartlist" v-bind:key="clist.index" class="list-group-item" style="text-align:center;">
                    <a v-bind:href="clist.xlssrc"><img src="/assets/img/excel.jpg" title="Excel 다운로드" border="0px" width="30"></a>
...
export default {
    ...
    data() {
         return {
            ...
            gubun3: 0,               // 중분류
            gubun4: "",              // 회원사명
            pbgubun2:3,              //PB권한의 경우 PB셀렉터가 바로 안보여서....오로지 화면에 PB로 잡혀있게 하기위해...
            grpgubun:{},
            allshow    :false,
    ...
    created: function () {
        ...
        var user_level = this.$store.state.user.user_level;
        
        if(machine_name.indexOf("ALL")>-1){                  // 조회회선이 ALL이면(모든회원 조회가능) 회원명을 보이게 하고, 
            // ALL이면 주문,시세,PB 다 볼수있음.
            this.allshow = true ;
            this.pbshow =false;
            this.normalshow =false;
        }else if(machine_name.indexOf("PB")>-1 || user_level=='pbcable'){
            this.pbshow =true;
            this.allshow = false ;
            this.normalshow =false;
        }else{
            // ALL이 없으면 자기것만 보는 일반유저              // 일반회원이면 안보이게 하기 위함. (col-md-6 과 col-md-4로 디자인이 이쁘지 않아서)
            this.normalshow = true ;
            this.allshow = false;
            this.pbshow =false;
            this.gubun4 = machine_name;                     //gubun4 = 조회케이블
        }
    },
    computed : {
        getMaxDate : function() {
            if(this.indate > this.indate1){
                ...
                this.indate1 = td_year + "-" + td_month + "-" + td_date ;
        ...
        middlelist : function() {
            //PB권한의 경우 PB셀렉터가 바로 안보여서....오로지 화면에 PB로 잡혀있게 하기위해...
            if(this.pbshow){
                this.gubun2 = 3;
                this.gubun3 = 1;
            }    
    ...
    methods: {
        ...
        onLargeChange: function() {
            ...
            //ALL 회원의 경우 대분류에서 주문, 시세, PB 를 볼수있는데, 주문, 시세 선택시 중분류 전체 선택가능/ PB 불가
            if(this.gubun2 == 0 || this.gubun2 == 1 || this.gubun2 == 2){
        ...
        fetchChartListTerm: function () {
            ...
            if(daydiff > 91 ){
                ...
                // console.log('일반유저인데 null이면 전체지....gubun4: '+this.gubun4)
                this.grpgubun = this.$store.state.user.grpgubun ;
                //PB권한의 경우 PB셀렉터가 바로 안보여서....오로지 화면에 PB로 잡혀있게 하기위해...
                if(this.pbshow){
                    this.gubun2 = this.pbgubun2;
                }
                axios.post(Config.base_url+'/user/chart/chartlistterm',{
                    "indate" : this.indate,
                    "indate1": this.indate1,
                    "gubun1" : this.gubun1,      //표기방식
                    "gubun2" : this.gubun2,      //대분류
                    "gubun3" : this.gubun3,      //중분류
                    "gubun4" : this.gubun4,      //회원조회가능케이블
                    "grpgubun": this.grpgubun   //
                }).then(function (response) {
...
.indate1normalmod{margin-left:20px;width:185px;}
